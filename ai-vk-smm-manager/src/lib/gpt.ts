...
function mockChatReply(message: string, context: string): string {
  ...
  if (/(лайк|охват|просмотр|подписчик)/.test(m)) {
    return `Текущая статистика вовлечённости: ${num("likes")} лайков, ${num("comments")} комментариев, ${num("views")} просмотров суммарно. Подписчиков: ${num("followers")} (+${rand(4, 18)} за сутки). Рекомендую держать расписание без пропусков — алгоритм ВК это любит.`;
  }
  return pick([
