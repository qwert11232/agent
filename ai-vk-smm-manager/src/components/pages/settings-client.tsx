        ...
        {postMode !== "schedule" ? (
          <div className="mt-5 border-t-[3px] border-line pt-4">
            <label className="field-label">
              Публиковать каждые: {intervalLabel(intervalMinutes)}
            </label>
            <div className="flex flex-wrap gap-2">
              {[30, 60, 120, 180, 240, 360, 480, 720, 1440].map((m) => (
                <button
                  key={m}
                  onClick={() => setIntervalMinutes(m)}
                  className={`btn btn-sm ${intervalMinutes === m ? "btn-violet" : "btn-ghost"}`}
                >
                  {intervalLabel(m)}
                </button>
              ))}
            ...
            <p className="mt-3 text-base text-muted">
              Бот сам генерирует и публикует пост, как только с прошлой публикации
              прошло выбранное время. Полностью автономная группа — расписание не нужно.
            </p>
          </div>
        ) : null}
      </Panel>
